...
def mode_ndle_manual_assist(allowed_words, n_boards, max_guesses, entropy_cache_path, n_jobs):
    ...
    while True:
        ...
        if guess_count == 0:
            ...
            top10 = top_multi_entropy_guesses(
                ...
                first_turn_cache=None,
                n_jobs=n_jobs,
                progress_label=f"[{n_boards}dle entropy]",
            )
        ...
        if guess_count >= max_guesses:
            print(f"\nReached max guesses ({n_boards}dle would be lost)")

        board_labels = " ".join([f"p{i+1}" for i in range(n_boards)])
        line = input(
            f"\nEnter: guess {board_labels}  (use '-' for solved boards) OR q: "
        ).strip().lower()
        if line == "q":
            return
        ...
        if len(parts) != 1 + n_boards:
            print(f"Format: guess {board_labels}")
            continue
...
def parse_args():
    ...
    ap.add_argument("--entropy-cache", default="first_guess_entropies.txt")
    ap.add_argument("--boards", type=int, default=32)
    ap.add_argument("--max-guesses", type=int, default=0)
...
def main():
    ...
    n_jobs = args.jobs if args.jobs and args.jobs > 0 else (os.cpu_count() or 1)

    if args.max_guesses and args.max_guesses > 0:
        max_guesses = args.max_guesses
    else:
        max_guesses = 37 if n_boards == 32 else (21 if n_boards == 16 else 6 + n_boards)

    mode_ndle_manual_assist(
        allowed_words=allowed,
        n_boards=n_boards,
        max_guesses=max_guesses,
        entropy_cache_path=args.entropy_cache,
        n_jobs=n_jobs,
    )
